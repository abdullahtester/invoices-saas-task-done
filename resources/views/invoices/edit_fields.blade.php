<div class="">
    <div class="d-flex flex-column align-items-start flex-xxl-row justify-content-between">
        ...
        <div class="d-flex flex-center flex-equal fw-row text-nowrap order-1 order-xxl-2 me-4 align-self-end align-items-center"
            ...
            <h4 class="align-items-center">Invoice # <span class="text-gray-500">{{ $invoice->invoice_id }}</span></h4>
        ...
        <div class="row gx-10 mb-5">
            <div class="col-lg-3 col-sm-12">
                <div class="mb-5">
                    ...
                    {{ Form::select('discount_type', $discount_type, isset($invoice) ? $invoice->discount_type : 0, ['class' =>'form-select io-select2', 'id' => 'discountType']) }}
                </div>
            </div>
            <div class="col-lg-3 col-sm-12">
                <div class="mb-5">
            ...
            <div class="mb-5 col-lg-6 col-sm-12">
                ...
                <select id="invoiceCurrencyType" class="form-select" name="currency_id">
                    ...
                    @foreach($currencies as $key => $currency)
                        <option value="{{ $currency['id'] }}" {{ $invoice->currency_id == $currency['id'] ? 'selected' : ''}}>{{$currency['icon']}}&nbsp;&nbsp;&nbsp; {{$currency['name']}}
                        </option>
                    @endforeach
                </select>
            </div>
                ...
                <div class="border-top">
                    <table class="table table-borderless box-shadow-none mb-0 mt-5">
                        ...
                        <tr>
                            ...
                            <td class="text-gray-900 text-end pe-0">
                                @if(!getSettingValue('currency_after_amount'))
                                    <span class="invoice-selected-currency">{{ getInvoiceCurrencySymbol($invoice->currency_id)  }}</span>@endif <span id="total" class="price">
                                    {{ isset($invoice) ? number_format($invoice->amount,2) : 0 }}
                            </span>
                                @if(getSettingValue('currency_after_amount'))
                                        <span class="invoice-selected-currency">{{ getInvoiceCurrencySymbol($invoice->currency_id)  }}</span>@endif
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td class="text-gray-900 text-end pe-0">
                                @if(!getSettingValue('currency_after_amount'))
                                    <span class="invoice-selected-currency">{{ getInvoiceCurrencySymbol($invoice->currency_id)  }}</span>@endif <span id="discountAmount">
                        ...
                        <tr>
                            ...
                            <td class="text-gray-900 text-end pe-0">
                                @if(!getSettingValue('currency_after_amount'))
                                    <span class="invoice-selected-currency">{{ getInvoiceCurrencySymbol($invoice->currency_id)  }}</span>@endif <span id="totalTax">
                                    0
                            </span>
                                @if(getSettingValue('currency_after_amount'))
                                        <span class="invoice-selected-currency">{{ getInvoiceCurrencySymbol($invoice->currency_id)  }}</span>@endif
                        ...
                        <tr>
                            ...
                            <td class="text-gray-900 text-end pe-0">
                                @if(!getSettingValue('currency_after_amount'))
                                    <span class="invoice-selected-currency">{{ getInvoiceCurrencySymbol($invoice->currency_id)  }}</span>@endif <span id="finalAmount">
